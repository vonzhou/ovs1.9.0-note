...
static int __init dp_init(void)
{
	...
	err = genl_exec_init();   // genl_exec.c 完成generic netlink family 的注册；
	if (err)
		goto error;

	err = ovs_workqueues_init(); //datapath/linux/compat/workqueue.c, 创建内核线程workq_thread 
	if (err)
		goto error_genl_exec;

	err = ovs_tnl_init();  //tunel.c， 初始化端口表（1024个）
	if (err)
		goto error_wq;

	err = ovs_flow_init();   // flow.c , 申请flow cache
	if (err)
		goto error_tnl_exit;
