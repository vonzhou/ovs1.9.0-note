 ...
 * Call this function between daemonize_start() and daemonize_complete(). */
 /*
在当前进程中启动一个worker process作为子进程，当前只支持一个，所以要确保只被调用一次
并且要在客户程序主循环中呼叫 worker_run() 和worker_wait() 。
worker_start 定义在lib/worker.c，工作流程：通过socketpair函数（仅适用于Unix域套接字）
创建一个流管道work_fds[0],work_fds[1]，然后通过fcntl设置为非阻塞状态；
接下来 fork_and_clean_up在fork子进程中做一些工作。
daemonize_post_detach()如果有守护进程的相关配置，那么这个方法就会响应这些，
即如果有 --detach ,--no-chdir的话那么detach=true，chdir_=false 
（关于这些定义在daemon.c中），然后关闭标准文件描述符。接下来就是子进程作为
RPC server和主进程沟通(child:work_fd[1]<->parent:work_fds[0]) 。
*/
void
worker_start(void)
{
